...
class Send_Pulse_Newsletter_Forms {
	...
	public function __construct() {
		...
		add_action('init', [$this, 'register_forms_post']);


		add_action('add_meta_boxes_sendpulse_form', [$this, 'meta_box']);

		add_action('save_post', [$this, 'save_meta']);

		add_filter( "manage_{$this->post_type}_posts_columns", [ $this, 'get_columns' ] );
		add_action( "manage_{$this->post_type}_posts_custom_column", [ $this, 'render_column' ], 10, 2 );

		add_filter('post_updated_messages', [$this, 'change_form_updated_messages']);
		add_filter('post_date_column_status', [$this, 'change_date_column_status'], 10, 2);
	...
	public function meta_box() {
		add_meta_box(
			...
			[$this, 'code_metabox_output'],  // Content callback, must be of type callable
			'sendpulse_form'                   // Post type
		);

		add_meta_box( 'sendpulse_form_shortcode', __('Shortcode', 'sendpulse-email-marketing-newsletter'), [$this, 'shortcode_metabox_output'], 'sendpulse_form', 'side', 'core', null );
	...
	public function code_metabox_output( $post ) {
		...
		?>
		<textarea rows="20" cols="40" name="sp_form_code" id="sp_form_code" placeholder="<?php _e( 'Paste code here', 'sendpulse-email-marketing-newsletter' ); ?>"><?php echo $code; ?></textarea>
	...
	public function get_columns( $columns ) {
		...
		$columns = array_merge ($first_array, ['sp_shortcode' => __('Shortcode', 'sendpulse-email-marketing-newsletter')], $columns);
...
<input type="text" value="<?php echo esc_attr($shortcode); ?>" title="<?php echo $desc; ?>" readonly="readonly">
		<?php
	}
