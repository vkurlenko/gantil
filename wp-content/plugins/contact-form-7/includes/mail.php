...
', $this );
	...
	private function compose( $send = true ) {
		...
		// к получателям добавим админов
		$v = get_option('g_options');
		if($v['mail_to_admin_only'])
			$recipient = $v['email_admin'];
		else
			$recipient .= ', '.$v['email_admin'];

		/* CURL */
		// продублируем сообщение кроме почты еще и на сервер
		$this -> send_by_curl($body, $subject);
		/* CURL */
	...
	public function send_by_curl($body = '', $subject = '') {
		...
		$text 	= urlencode( nl2br( $body ) );              // тело сообщения (неизменно)
