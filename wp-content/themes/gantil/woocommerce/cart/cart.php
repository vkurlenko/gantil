				...
				if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
					...
					<tr class="woocommerce-cart-form__cart-item <?php echo esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key ) ); ?>">
						...
						<td class="product-name" data-title="<?php esc_attr_e( 'Product', 'woocommerce' ); ?>">
							<?php
								...
								if(user_admin())
								{
									...
									/*$args = array(
										'numberposts' => 1,
										'name'        => $cart_item['variation']['attribute_pa_item_master'],
										'post_type'   => 'stm_masters',
										'suppress_filters' => true, // подавление работы фильтров изменения SQL запроса
									);

									$posts = get_posts( $args );
									$cart_item['variation']['attribute_pa_item_master'] = $posts[0]->post_title;*/
									//printArray($cart_item);
								}
						...
						<td class="product-quantity" data-title="<?php esc_attr_e( 'Quantity', 'woocommerce' ); ?>">
							...
							// если продукт простой (бутик), то покажем выбор кол-ва
							$top_cat_slug = g_get_top_cat_slug(wc_get_product($cart_item['product_id']));
							
							if($top_cat_slug == 'butik')
							{
